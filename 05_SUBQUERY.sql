...
WHERE SALARY > (SELECT AVG(SALARY) FROM EMPLOYEE) --평균급여
ORDER BY JOB_CODE;
...
--WHER EXISTS (서브쿼리)
--> 서브쿼리 결과(RESULT SET)에 행이 존재 한다면 TRUE
-- 없으면 FALSE
----------------------------------------------------------------------------------
...
-- 3) ROWNUM을 이용서 급여 상위 5명 조회
SELECT ROWNUM, EMP_NAME, SALARY
FROM EMPLOYEE
ORDER BY SALARY DESC;
-- [문제 발생] --
--> SELECT절이 ORDER BY보다 먼저 해석되어
-- ROWNUM 순서가 급여 순서가 아닌 ,조회된 행 순서로 지정됨
...
SELECT EMP_ID , EMP_NAME ,JOB_NAME , 
       SUBSTR(EMP_NO,1,2) "나이", SALARY * 12 "보너스포함연봉"  
FROM EMPLOYEE
JOIN JOB USING (JOB_CODE)
