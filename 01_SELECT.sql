...
SELECT EMP_NAME, SALARY  ,SALARY + 10000000
FROM EMPLOYEE;
...
-- SYSTIMESTAMP : 현제 시간(년,월,일,시,분,초,ms + 지역(local)) 조회
...
-- 현재 시간 -어제 0시 0분 0초
SELECT TO_DATE('2024-02-25','YYYY-MM-DD')
FROM DUAL;
...
-- CEIL(숫자): 올림 처리
-- 현재시간 - 생년월일
SELECT CEIL( (SYSDATE - TO_DATE('2002-11-04','YYYY-MM-DD'))/365)
FROM DUAL;
...
SELECT
   CEIL( (SYSDATE - TO_DATE('2002-11-04','YYYY-MM-DD'))/365) AS 나이
FROM DUAL;
...
ORDER BY DEPT_CODE; -- ASC 생략 가능(기본값이다!!!)
...
-- SELECT절에서 해석된 별칭을 ORDER BY절에서 사용 할 수 있다.
...
FROM EMPLOYEE
ORDER BY DEPT_CODE, JOB_CODE DESC, EMP_NAME ASC; 
