...
-- 여러 SQL 한 번에 수행하기 : 블록으로 선택 -> ALT + X
...
-- 가입일 -> SYSDATE를 활용
INSERT INTO "MEMBER"
VALUES('MEM02', 'QWER1234', '김영희', '980808-2345678', SYSDATE);
...
CREATE TABLE USER_USED_UK(
   ...
   -- USER_ID VARCHAR2(20) UNIQUE, -- 컬럼 레벨 제약 조건 (이름 x)
    ...
    -- 테이블 레벨 --
    UNIQUE(USER_NAME) -- 테이블 레벨 제약 조건 설정(이름 X)
...
-- SQL Error [1] [23000]: ORA-00001: 무결성 제약 조건(KH_JTG.USER_ID_U)에 위배됩니다.
...
--> 기본키 중복으로 오류
-- SQL Error [1] [23000]: ORA-00001: 무결성 제약 조건(KH_JTG.USER_NO_PK)에 위배됩니다

INSERT INTO USER_USED_PK
VALUES(NULL, 'user03', 'pass03', '유관순', '여', '010-9999-3131', 'yoo123@kh.or.kr');
--> 기본키가 NULL 이므로 오류
-- SQL Error [1400] [23000]: ORA-01400: NULL을 ("KH_JTG"."USER_USED_PK"."USER_NO") 안에 삽입할 수 없습니다
...
CREATE TABLE USER_USED_FK(
  ...
  GRADE_CODE NUMBERCONSTRAINT GRADE_CODE_FK1 REFERENCES USER_GRADE(GRADE_CODE)
  												-- 컬럼 레벨
...
-- - 부모 키가 없습니다
 --> 참조하는 테이블(USER_GRADE)에 값이 존재하지 않음
...
CREATE TABLE USER_USED_FK2(
  ...
  ON DELETE SET NULL  -- 부모키가 삭제되면 참조하던 자식의 값을 NULL로 변경
...
SELECT * FROM USER_GRADE2;
SELECT * FROM USER_USED_FK2; --10을 참조하던 컬럼이 NULL로 변함
...
CREATE TABLE USER_USED_FK3(
  ...
  ON DELETE  CASCADE -- 부모키 삭제 시 참조하는 자식의 행도 모두 삭제
...
-- SQL Error [2290] [23000]: ORA-02290: 체크 제약조건(KH_JTG.GENDER_CHECK)이 위배되었습니다
...
-- 2) JOIN 후 원하는 컬럼만 테이블로 복사 x
-- + DEFAULT, COMMENT 복사 X
